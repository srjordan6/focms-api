...
class AffiliationItem(BaseModel):
    ...
    # v0.12.139: usa_member checkbox - True writes member:true + fields (wholesale
    # replace of the usa_swimming key), False removes the key entirely.
    usa_member: Optional[bool] = None
            ...
            for it in body.items or []:
                ...
                if it.id:
                    ...
                    r = await conn.execute(
                        ...
                        "details = (CASE WHEN $23::boolean IS FALSE THEN details - 'usa_swimming' ELSE details END) "
                        "|| CASE WHEN $19::text IS NULL THEN '{}'::jsonb "
                        "ELSE jsonb_build_object('program_code', $19::text) END "
                        "|| CASE WHEN $23::boolean IS TRUE OR $20::text IS NOT NULL OR $21::text IS NOT NULL OR $22::text IS NOT NULL THEN "
...
class SchoolProfileItem(BaseModel):
    ...
    # v0.12.162: the federal school id, shown on the record. For PUBLIC schools
    # that is the 12-digit NCES/CCD ncessch (already carried in school_leaid);
    # for PRIVATE schools it is the PSS id (e.g. A1703455), which has letters and
    # therefore cannot live in the digits-only leaid the district cascade slices.
...
async def get_school_profiles(request: Request, student_id: str):
    ...
    async with _tenant_conn(pool, tenant_id) as conn:
        rows = await conn.fetch(
            ...
            "school_phone, school_fax, principal_name, principal_phone, nces_school_id, "
            ...
            for it in body.items or []:
                ...
                if it.id:
                    ...
                    await conn.execute(
                        ...
                        "graduating_class_size, boarding_students, curriculum_notes, notes, "
                        "visibility, source_system, created_by, updated_by) "
                        "VALUES ($1::uuid,$2::uuid,$3,$31,$32,$33,$34,$35,$4,$5,$6,$7,$36,$8,$9,$10,$37,$38,$39,$40,$41,$11,$12,$13,$14,"

#!/usr/bin/env python3
"""
focms_nightly_jobs.py v0.1.0 (2026-07-09)

Single Render Cron entry point - runs every scheduled job in sequence, each
isolated in its own subprocess so one failure never blocks the others.
Exits non-zero if ANY job failed so the Render run shows red.

Schedule: 0 9 * * * (daily 09:00 UTC). Manual Build after every code change.
